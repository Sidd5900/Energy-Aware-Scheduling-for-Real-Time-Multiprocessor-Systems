...
bool schedule_edf(int task_num, int replica_num)
{
    ...
    // put the task instances with even replica_num (they will be the primary copies) into the global_queue
        ...
        for(int k=0;k<m;k++)
        {
            ...
            if(!ready_queue[k].empty())
            {
                ...
                if(finish_time <= temp.arrival_time + temp.deadline)
                {
                    ...
                    // push the backup copy if any (the current copy should be primary) into the global queue
