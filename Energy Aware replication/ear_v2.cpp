...
bool schedule_edf(int task_num, int replica_num)
{
    ...
    {
        // make a scheduling decision at the finish of any of the tasks
        if(!finish_times.empty() && (*finish_times.begin()) <= global_queue.top().activation_time)
        {
    ...
    for(int k=0;k<m;k++)
    {
        while(!ready_queue[k].empty())
        {
            ...
            if(finish_time <= temp.arrival_time + temp.deadline)
            {
                Schedule sch = Schedule(cur_time, finish_time, tn, rn);
                schedule[k].push_back(sch);
                last_idle_time[k] = finish_time;
            }
            else
