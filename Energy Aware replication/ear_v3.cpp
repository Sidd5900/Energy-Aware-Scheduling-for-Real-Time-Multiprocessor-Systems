...
// finish_times contains a set of time instances when the tasks which are already scheduled finishes
int n,m,F;
vector<Task> task;
...
bool schedule_edf(int task_num, int replica_num)
{
    ...
    {
        for(int j=0;j<task[i].replicas;j+=2)
        {
            if(i==task_num && j>replica_num)
            {
                break;
            }
            double cur_time = 0;
    ...
    while(!global_queue.empty() || !finish_times.empty())
    {
         // make a scheduling decision at the finish of any of the tasks or at the arrival of a new task
        if(global_queue.empty())
        {
            global_time = (*finish_times.begin());
            finish_times.erase(finish_times.begin());
        }
        else if(!finish_times.empty() && (*finish_times.begin()) <= global_queue.top().activation_time)
        {
            global_time = (*finish_times.begin());
            finish_times.erase(finish_times.begin());
        }
        ...
        for(int k=0;k<m;k++)
        {
            ...
            if(!ready_queue[k].empty())
            {
                ...
                if(finish_time <= temp.arrival_time + temp.deadline)
                {
                    ...
                    finish_times.insert(finish_time);

                    // push the backup copy if any into the global queue
                    if(rn%2 == 0 && rn != temp.replicas-1 && (tn != task_num || rn+1 <= replica_num))
                    {
