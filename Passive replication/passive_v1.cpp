...
bool schedule_edf(int task_num, int replica_num)
{
    ...
    for(int i=0;i<=task_num;i++)
    {
        ...
        while(cur_time < hyperperiod)
        {
            ...
            // cout<<temp.tid<<" "<<temp.replica_num<<" "<<temp.activation_time<<" "<<temp.period<<endl;
        }
    }
    // cout<<"g: "<<global_queue.size()<<endl;
    /*
    while(!global_queue.empty())
    {
        Task temp = global_queue.top();
        global_queue.pop();
        cout<<temp.tid<<" "<<temp.replica_num<<" "<<temp.activation_time<<endl;
    }
    */
    ...
    while(!global_queue.empty())
    {
        ...
        while(!global_queue.empty() && global_queue.top().activation_time <= global_time)
        {
            ...
            // cout<<"tn rn alloc "<<tn<<" "<<rn<<" "<<alloc<<endl;
        }
        // cout<<"bye"<<endl;
            ...
            if(!ready_queue[k].empty())
            {
                ...
                if(finish_time <= temp.arrival_time + temp.deadline)
                {
                    ...
                    if(rn != temp.replicas-1 && rn+1 <= replica_num)
                    {
                        ...
                        // cout<<"next "<<next_replica.replica_num<<" "<<next_replica.activation_time<<endl;
                    }
                }
                else
                {
